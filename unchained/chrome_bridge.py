...
class Agent:
    ...
    async def _handle_message(self, msg: dict):
        ...
        elif t == "overlay_inject":
            await self._handle_overlay_inject(msg)
        elif t == "overlay_event":
            await self._handle_overlay_event(msg)
    ...
    # --- Overlay copilot (injected locally via CDP) ---

    _overlay_ws: dict = {}       # session_id → websockets CDP connection
    ...
    async def _handle_overlay_inject(self, msg: dict):
        ...
        tab_id = msg.get("tab_id", "")
        overlay_js = msg.get("overlay_js", "")
        bootstrap_js = msg.get("bootstrap_js", "")
        nonce = msg.get("nonce", "")
        prompt = msg.get("prompt", "")
        if not session_id or not tab_id or not overlay_js:
            return

        # Cancel previous overlay for this session
        old_task = self._overlay_poll_task.pop(session_id, None)
        if old_task and not old_task.done():
            old_task.cancel()
        old_ws = self._overlay_ws.pop(session_id, None)
        if old_ws:
            try:
                await old_ws.close()
            except Exception:
        ...
        try:
            ws_url = self._get_tab_ws_url(tab_id, channel=-1)
            chrome_ws = await websockets.connect(ws_url, max_size=50 * 1024 * 1024)
            self._overlay_ws[session_id] = chrome_ws
            self._overlay_session[session_id] = {
                "tab_id": tab_id, "nonce": nonce, "prompt": prompt,
            }
            ...
            # Start outbox poller
            task = asyncio.create_task(self._overlay_poll_outbox(session_id, chrome_ws, nonce))
            self._overlay_poll_task[session_id] = task

        except Exception as e:
            log.warning("[overlay] Injection failed for %s: %s", session_id, e)

    async def _handle_overlay_event(self, msg: dict):
        """Push a chat event to the overlay panel via local CDP."""
        session_id = msg.get("session_id", "")
        event = msg.get("event", {})
        chrome_ws = self._overlay_ws.get(session_id)
        if not chrome_ws:
            return
        try:
    ...
    async def _overlay_poll_outbox(self, session_id: str, chrome_ws, nonce: str):
        """Poll the overlay JS outbox for follow-up messages."""
        poll_js = "(function(){var q=window.__uc_overlay_outbox||[];window.__uc_overlay_outbox=[];return JSON.stringify(q)})()"
        try:
            while True:
                ...
                try:
                    ...
                    if raw and raw != "[]":
                        msgs = json.loads(raw)
                        for m in msgs:
                            if m.get("type") != "user_followup":
                                continue
                            if m.get("nonce") != nonce:
                                log.warning("[overlay] follow-up rejected — bad nonce")
                                continue
                            text = str(m.get("message", "")).strip()
                            if text and len(text) <= 4000:
                                # Send follow-up back to server via relay WS
                                await self.ws.send(json.dumps({
                                    "type": "overlay_followup",
                                    "session_id": session_id,
                                    "message": text,
                                }))
                                log.info("[overlay] follow-up from %s: %s", session_id, text[:60])
            ...
            self._overlay_ws.pop(session_id, None)
            self._overlay_session.pop(session_id, None)
            self._overlay_poll_task.pop(session_id, None)
