...
async def cdp_screenshot(tab_id: str = "auto", agent_id: str = "") -> Image:
    ...
    try:
        ...
        # Screenshot failed — fall back to DDM text as a text tool response
        try:
            ddm_text = await cloud_tools.run_ddm(aid, tab_id, ["--text"])
            # Return DDM as a minimal 1x1 PNG with text in metadata is not practical.
            # Re-raise with helpful context instead.
            raise RuntimeError(
