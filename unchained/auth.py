...
class Auth:
            ...
            conn.execute("""
                CREATE TABLE IF NOT EXISTS lifecycle_email_deliveries (
                    ...
                    status TEXT NOT NULL CHECK (
                        status IN ('reserved', 'smtp_accepted', 'failed', 'cancelled')
                    ),
                    created_at REAL NOT NULL,
        ...
        *,
        ...
        with self._conn() as conn:
            eligible = conn.execute(
                "SELECT 1 FROM lifecycle_email_deliveries d "
                "JOIN users u ON u.user_id = d.user_id "
                "LEFT JOIN lifecycle_email_suppressions s ON s.user_id = d.user_id "
                "WHERE d.user_id = ? AND d.campaign = ? AND d.status = 'reserved' "
                "AND u.first_install_bootstrap_at IS NULL AND s.user_id IS NULL",
                (uid, campaign_name),
            ).fetchone()
    ...
    def mark_lifecycle_email_delivery(
        ...
        *,
        ...
        """Finalize a reserved lifecycle send once after an SMTP attempt."""
        state = str(status or "").strip().lower()
        if state not in {"smtp_accepted", "failed", "cancelled"}:
            raise ValueError("status must be smtp_accepted, failed, or cancelled")
