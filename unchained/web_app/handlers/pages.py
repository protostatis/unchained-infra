...
<body>
  ...
  <script>
    ...
    const MAX_TRANSIENT_HANDSHAKE_NOT_READY = 2;
    ...
    function restorePendingHandshake(){const saved=sessionJson(PENDING_HANDSHAKE_KEY);const safeStatusUrl=safeOptionalLocalUrl(saved?.status_url||'');const safeRequestId=String(saved?.request_id||'').trim();const storedAtEpoch=Number(saved?.stored_at_epoch||0);if(!safeStatusUrl||!safeRequestId){clearPendingHandshake();return null;}handshakeInFlight=true;const elapsedSeconds=Number.isFinite(storedAtEpoch)&&storedAtEpoch>0 ? Math.max(0, (Date.now()/1000)-storedAtEpoch) : 0;const startAttempt=Math.min(Math.floor(elapsedSeconds/(HANDSHAKE_POLL_INTERVAL_MS/1000)), MAX_HANDSHAKE_POLL_ATTEMPTS);return {statusUrl:safeStatusUrl,requestId:safeRequestId,startAttempt};}
    ...
    function persistMissionWatchState(url, data){const safeUrl=safeOptionalLocalUrl(url);if(!safeUrl){clearMissionWatchState(false);return;}missionWatchUrl=safeUrl;setSessionValue(MISSION_WATCH_URL_KEY,safeUrl);/* Preserve the existing snapshot while the watch URL stays the same; URL changes intentionally drop the old snapshot. */if(data&&data.ok){setSessionJson(MISSION_WATCH_STATE_KEY,{url:safeUrl,data});}}
