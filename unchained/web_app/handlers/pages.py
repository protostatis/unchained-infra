...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;color:var(--text);font-family:"Iowan Old Style","Palatino Linotype","Book Antiqua",serif;background:radial-gradient(circle at top left, rgba(123,224,184,0.12), transparent 32%),radial-gradient(circle at top right, rgba(244,197,92,0.10), transparent 24%),linear-gradient(180deg,#071018 0%,#08111b 55%,#050a0f 100%)}
    ...
    .capsule-card h3{margin:0 0 6px;font-size:20px}.capsule-meta{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}code{font-family:ui-monospace,SFMono-Regular,Menlo,Consolas,monospace;background:rgba(255,255,255,0.06);padding:2px 6px;border-radius:6px}
...
<body>
  ...
  <script>
    ...
    function setLaunchReady(ready, href){const link=document.getElementById('open-local-desk');if(!link) return;if(href) link.href=href;link.classList.toggle('disabled', !ready);link.setAttribute('aria-disabled', ready ? 'false' : 'true');if(ready){link.removeAttribute('tabindex');}else{link.setAttribute('tabindex','-1');}}
    function renderStatus(data){const title=document.getElementById('status-title');const copy=document.getElementById('status-copy');const chips=document.getElementById('status-chips');const launchUrl=String(data.local_urls?.home||'http://127.0.0.1:8766/');const configuredProvider=String(data.provider?.configured_provider||'unknown');const browserClient=String(data.provider?.browser_client||'');const trialStatus=String(data.trial?.status||'unknown');chips.innerHTML='';title.textContent=data.launch_ready ? 'Local Research Desk detected.' : 'Local Research Desk found, but setup is incomplete.';copy.textContent='Hosted Unchained can see the local status surface. Use the local app for the full workflow and local notebook execution.';chips.appendChild(chip('provider: '+configuredProvider));chips.appendChild(chip('agent mode: '+String(data.provider?.agent_mode||'unknown')));if(browserClient) chips.appendChild(chip('browser client: '+browserClient));chips.appendChild(chip('trial: '+trialStatus));chips.appendChild(chip('bridge key: '+(data.bridge?.api_key_present?'present':'missing')));chips.appendChild(chip('agent: '+String(data.bridge?.agent_id||'not found')));chips.appendChild(chip('pyreplab: '+(data.pyreplab?.available?'ready':'missing')));chips.appendChild(chip('capsules: '+String(data.capsules?.count||0)));if(Array.isArray(data.missing) && data.missing.length) chips.appendChild(chip('missing: '+data.missing.join(', ')));setLaunchReady(Boolean(data.launch_ready), launchUrl);}
