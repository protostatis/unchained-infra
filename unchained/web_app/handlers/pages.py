...
<body>
  ...
  <script>
    ...
    function scheduleHandshakePoll(statusUrl, requestId){if(handshakePollTimer) window.clearTimeout(handshakePollTimer);handshakePollTimer=window.setTimeout(async()=>{try{const resp=await fetch(statusUrl+'?request_id='+encodeURIComponent(requestId),{mode:'cors'});if(!resp.ok) throw new Error('handshake status unavailable');const data=await resp.json();if(data.status==='approved'){approvedHandshakeToken=String(data.session_token||'');setMissionCreateReady(Boolean(approvedHandshakeToken));setConnectNote('Hosted connection approved. Local session token is ready for the next action.');return;}if(data.status==='denied'){approvedHandshakeToken='';setMissionCreateReady(false);setConnectNote('Hosted connection was denied in the local desk.');return;}scheduleHandshakePoll(statusUrl, requestId);}catch(_err){setConnectNote('Waiting for local approval...');scheduleHandshakePoll(statusUrl, requestId);}},1500);}
    ...
    document.getElementById('connect-local-desk')?.addEventListener('click', async()=>{if(!latestDeskStatus?.handshake?.start_url||!latestDeskStatus?.handshake?.status_url) return;setConnectNote('Requesting local approval...');try{const body=new URLSearchParams({client_label:'Unchained First Look',requested_scope:'mission:create'});const resp=await fetch(String(latestDeskStatus.handshake.start_url),{method:'POST',mode:'cors',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});if(!resp.ok) throw new Error('handshake start failed');const data=await resp.json();if(data.approval_url) window.open(String(data.approval_url),'_blank','noopener');setConnectNote('Approve the request in the local desk tab that just opened.');if(data.request_id) scheduleHandshakePoll(String(latestDeskStatus.handshake.status_url),String(data.request_id));}catch(_err){setConnectNote('Could not start the local approval flow. Make sure Research Desk is still running, then try again.');}});
    document.getElementById('create-local-mission')?.addEventListener('click', async()=>{if(!approvedHandshakeToken||!latestDeskStatus?.handshake?.actions?.mission_create_url) return;setConnectNote('Creating a local Mission from hosted Unchained...');try{const body=new URLSearchParams({mission_prompt:'Continue this First Look in Research Desk'});const resp=await fetch(String(latestDeskStatus.handshake.actions.mission_create_url),{method:'POST',mode:'cors',headers:{'Content-Type':'application/x-www-form-urlencoded','Authorization':'Bearer '+approvedHandshakeToken},body});const data=await resp.json();if(!resp.ok||!data.ok) throw new Error('mission create failed');if(data.mission_url){const missionUrl=safeLocalUrl(FALLBACK_LOCAL_URL.slice(0,-1)+String(data.mission_url));window.open(missionUrl,'_blank','noopener');}setConnectNote('Local Mission created successfully. Research Desk opened it in a new tab.');}catch(_err){setConnectNote('Could not create the local Mission. Try reconnecting to the local desk first.');}});
