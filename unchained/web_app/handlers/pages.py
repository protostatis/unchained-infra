...
<body>
  ...
  <script>
    ...
    function currentPromptFromContext(){try{const qs=new URLSearchParams(window.location.search);const prompt=normalizeHandoffPrompt(qs.get('prompt')||'');if(prompt) return prompt;return normalizeHandoffPrompt(window.localStorage.getItem(FIRST_LOOK_PROMPT_KEY)||'');}catch(_err){return '';}}
