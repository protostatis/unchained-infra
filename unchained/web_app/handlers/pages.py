...
<body>
  ...
  <script>
    ...
    function currentPromptFromContext(){try{const qs=new URLSearchParams(window.location.search);const prompt=String(qs.get('prompt')||'').trim();if(prompt) return prompt;return String(window.localStorage.getItem(FIRST_LOOK_PROMPT_KEY)||'').trim();}catch(_err){return '';}}
    function currentSourceSessionId(){try{const qs=new URLSearchParams(window.location.search);const value=String(qs.get('session_id')||'').trim();if(value) return value;return String(window.localStorage.getItem(FIRST_LOOK_SESSION_KEY)||'').trim();}catch(_err){return '';}}
    function renderMissionWatch(data){const card=document.getElementById('mission-watch');const title=document.getElementById('mission-watch-title');const copy=document.getElementById('mission-watch-copy');const meta=document.getElementById('mission-watch-meta');const link=document.getElementById('mission-watch-link');if(!card||!title||!copy||!meta||!link) return;if(!data||!data.ok){card.classList.remove('visible');missionWatchUrl='';missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';if(missionWatchTimer){window.clearTimeout(missionWatchTimer);missionWatchTimer=null;}return;}card.classList.add('visible');title.textContent=String(data.capsule_name||'mission');copy.textContent=String(data.next_step||'Mission is progressing in the local desk.');meta.innerHTML='';[String(data.stage||'planning'), String(data.readiness_status||'planned'), String((Number(data.primary_row_count||0))+' rows')].forEach((value)=>{const badge=document.createElement('span');badge.className='pill';badge.textContent=value;meta.appendChild(badge);});link.href=safeLocalUrl(data.mission_url_abs||data.mission_url||FALLBACK_LOCAL_URL);link.classList.remove('disabled');link.setAttribute('aria-disabled','false');link.removeAttribute('tabindex');}
    function scheduleMissionWatch(url){if(missionWatchTimer) window.clearTimeout(missionWatchTimer);const safeUrl=safeOptionalLocalUrl(url);if(!safeUrl){missionWatchUrl='';missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';return;}if(missionWatchUrl!==safeUrl){missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';}missionWatchUrl=safeUrl;if(missionWatchAttempts>=MAX_MISSION_WATCH_ATTEMPTS){setConnectNote('Mission watch timed out. Open the local desk to continue from there.');return;}missionWatchTimer=window.setTimeout(async()=>{missionWatchTimer=null;missionWatchAttempts+=1;try{const resp=await fetch(safeUrl,{mode:'cors',credentials:'omit',cache:'no-store',referrerPolicy:'no-referrer',signal:AbortSignal.timeout(5000)});if(!resp.ok){setConnectNote('Mission watch stopped after repeated local status failures. Open the local desk to continue.');return;}const data=await resp.json();renderMissionWatch(data);if(!data.ok) return;const readiness=String(data.readiness_status||'');const signature=[String(data.stage||''),readiness,String(data.primary_row_count||0),String(data.next_step||'')].join('|');if(signature===missionWatchLastSignature){missionWatchStableCount+=1;}else{missionWatchLastSignature=signature;missionWatchStableCount=0;}if(['final_ready','blocked'].includes(readiness)) return;if(missionWatchStableCount>=MAX_IDENTICAL_MISSION_STATES){setConnectNote('Mission watch paused because the local state stopped changing. Open the local desk to continue.');return;}scheduleMissionWatch(safeUrl);}catch(_err){if(missionWatchUrl===safeUrl&&missionWatchAttempts<MAX_MISSION_WATCH_ATTEMPTS){scheduleMissionWatch(safeUrl);return;}setConnectNote('Mission watch stopped after repeated local status failures. Open the local desk to continue.');}},MISSION_WATCH_POLL_INTERVAL_MS);}
