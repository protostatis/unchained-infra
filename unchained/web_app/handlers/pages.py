...
<body>
  ...
  <script>
    ...
    function setConnectReady(ready){const button=document.getElementById('connect-local-desk');if(!button) return;button.classList.toggle('disabled', !ready);button.setAttribute('aria-disabled', ready ? 'false' : 'true');button.disabled=!ready;}
    ...
    function scheduleHandshakePoll(statusUrl, requestId, attempt){if(handshakePollTimer) window.clearTimeout(handshakePollTimer);const tries=Number(attempt||0);if(tries>=MAX_HANDSHAKE_POLL_ATTEMPTS){handshakeInFlight=false;setConnectNote('Timed out waiting for local approval. Try connecting again.');setMissionCreateReady(false);return;}handshakePollTimer=window.setTimeout(async()=>{try{const pollUrl=new URL(statusUrl);pollUrl.searchParams.set('request_id', requestId);const resp=await fetch(pollUrl.toString(),{mode:'cors',signal:AbortSignal.timeout(5000)});if(!resp.ok){handshakeInFlight=false;setMissionCreateReady(false);setConnectNote('Local approval check failed. Try connecting again.');return;}const data=await resp.json();if(data.status==='approved'){handshakeInFlight=false;approvedHandshakeToken=sanitizeBearerToken(data.session_token);setMissionCreateReady(Boolean(approvedHandshakeToken)&&Boolean(safeOptionalLocalUrl(latestDeskStatus?.handshake?.actions?.mission_create_url)));setConnectNote('Hosted connection approved. Local session token is ready for the next action.');return;}if(data.status==='denied'){handshakeInFlight=false;approvedHandshakeToken='';setMissionCreateReady(false);setConnectNote('Hosted connection was denied in the local desk.');return;}setConnectNote('Waiting for local approval...');scheduleHandshakePoll(statusUrl, requestId, tries+1);}catch(_err){setConnectNote('Waiting for local approval...');scheduleHandshakePoll(statusUrl, requestId, tries+1);}},HANDSHAKE_POLL_INTERVAL_MS);}
    ...
    document.getElementById('connect-local-desk')?.addEventListener('click', async()=>{if(handshakeInFlight) return;const startUrl=safeOptionalLocalUrl(latestDeskStatus?.handshake?.start_url);const statusUrl=safeOptionalLocalUrl(latestDeskStatus?.handshake?.status_url);if(!startUrl||!statusUrl) return;handshakeInFlight=true;approvedHandshakeToken='';setMissionCreateReady(false);setConnectNote('Requesting local approval...');try{const body=new URLSearchParams({client_label:'Unchained First Look',requested_scope:'mission:create'});const resp=await fetch(startUrl,{method:'POST',mode:'cors',headers:{'Content-Type':'application/x-www-form-urlencoded'},body});if(!resp.ok) throw new Error('handshake start failed');const data=await resp.json();const approvalUrl=safeOptionalLocalUrl(data.approval_url);if(approvalUrl){window.open(approvalUrl,'_blank','noopener');setConnectNote('Approve the request in the local desk tab that just opened.');}else{setConnectNote('Approval tab could not be opened. Open the local desk manually and approve the pending request there.');}if(data.request_id) scheduleHandshakePoll(statusUrl,String(data.request_id),0);else handshakeInFlight=false;}catch(_err){handshakeInFlight=false;setConnectNote('Could not start the local approval flow. Make sure Research Desk is still running, then try again.');}});
    document.getElementById('create-local-mission')?.addEventListener('click', async()=>{const missionCreateUrl=safeOptionalLocalUrl(latestDeskStatus?.handshake?.actions?.mission_create_url);const safeToken=sanitizeBearerToken(approvedHandshakeToken);if(!safeToken||!missionCreateUrl) return;setConnectNote('Creating a local Mission from hosted Unchained...');try{const body=new URLSearchParams({mission_prompt:'Continue this First Look in Research Desk'});const resp=await fetch(missionCreateUrl,{method:'POST',mode:'cors',headers:{'Content-Type':'application/x-www-form-urlencoded','Authorization':'Bearer '+safeToken},body});const data=await resp.json();if(!resp.ok||!data.ok) throw new Error('mission create failed');if(data.mission_url){const missionUrl=safeLocalUrl(new URL(String(data.mission_url), FALLBACK_LOCAL_URL).toString());window.open(missionUrl,'_blank','noopener');}setConnectNote('Local Mission created successfully. Research Desk opened it in a new tab.');}catch(_err){setConnectNote('Could not create the local Mission. Try reconnecting to the local desk first.');}});
    ...
    window.addEventListener('beforeunload', ()=>{clearDeskProbe();if(handshakePollTimer){window.clearTimeout(handshakePollTimer);handshakePollTimer=null;}});
    loadDeskState();
  </script>
</body>
</html>"""
