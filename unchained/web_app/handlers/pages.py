...
<body>
  ...
  <script>
    ...
    function scheduleHandshakePoll(statusUrl, requestId){if(handshakePollTimer) window.clearTimeout(handshakePollTimer);handshakePollTimer=window.setTimeout(async()=>{try{const resp=await fetch(statusUrl+'?request_id='+encodeURIComponent(requestId),{mode:'cors'});if(!resp.ok) throw new Error('handshake status unavailable');const data=await resp.json();if(data.status==='approved'){setConnectNote('Hosted connection approved. Local session token is ready for the next action.');return;}if(data.status==='denied'){setConnectNote('Hosted connection was denied in the local desk.');return;}scheduleHandshakePoll(statusUrl, requestId);}catch(_err){setConnectNote('Waiting for local approval...');scheduleHandshakePoll(statusUrl, requestId);}},1500);}
