...
<body>
  ...
  <script>
    ...
    function persistApprovedHandshake(token, expiresAtEpoch){const safeToken=sanitizeBearerToken(token);const expiresAt=Number(expiresAtEpoch||0);if(!safeToken||!Number.isFinite(expiresAt)||expiresAt<=Date.now()/1000){clearApprovedHandshake();return '';}approvedHandshakeToken=safeToken;setSessionValue(HANDSHAKE_TOKEN_KEY,safeToken);setSessionValue(HANDSHAKE_TOKEN_EXPIRES_KEY,String(expiresAt));return safeToken;}
    ...
    function clearPendingHandshake(){handshakeInFlight=false;setSessionJson(PENDING_HANDSHAKE_KEY,null);}
    function persistPendingHandshake(statusUrl, requestId){const safeStatusUrl=safeOptionalLocalUrl(statusUrl);const safeRequestId=String(requestId||'').trim();if(!safeStatusUrl||!safeRequestId){clearPendingHandshake();return;}setSessionJson(PENDING_HANDSHAKE_KEY,{status_url:safeStatusUrl,request_id:safeRequestId,stored_at_epoch:Date.now()/1000});}
    function restorePendingHandshake(){const saved=sessionJson(PENDING_HANDSHAKE_KEY);const safeStatusUrl=safeOptionalLocalUrl(saved?.status_url||'');const safeRequestId=String(saved?.request_id||'').trim();if(!safeStatusUrl||!safeRequestId){clearPendingHandshake();return null;}handshakeInFlight=true;return {statusUrl:safeStatusUrl,requestId:safeRequestId};}
    function clearMissionWatchState(resetView){setSessionValue(MISSION_WATCH_URL_KEY,'');setSessionJson(MISSION_WATCH_STATE_KEY,null);missionWatchUrl='';missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';if(resetView) renderMissionWatch(null);}
    function persistMissionWatchState(url, data){const safeUrl=safeOptionalLocalUrl(url);if(!safeUrl){clearMissionWatchState(false);return;}missionWatchUrl=safeUrl;setSessionValue(MISSION_WATCH_URL_KEY,safeUrl);if(data&&data.ok){setSessionJson(MISSION_WATCH_STATE_KEY,{url:safeUrl,data});}}
    function restoreMissionWatchState(){const safeUrl=safeOptionalLocalUrl(sessionValue(MISSION_WATCH_URL_KEY));const saved=sessionJson(MISSION_WATCH_STATE_KEY);if(!safeUrl){clearMissionWatchState(false);return false;}missionWatchUrl=safeUrl;if(saved&&saved.data&&saved.data.ok){renderMissionWatch(saved.data);}return true;}
    function restoreStoredDeskSession(){const pending=restorePendingHandshake();const restoredToken=restoreApprovedHandshake();const restoredWatch=restoreMissionWatchState();if(pending){setConnectNote('Resuming the pending local approval check from your last hosted session...');scheduleHandshakePoll(pending.statusUrl, pending.requestId, 0);return;}if(restoredToken&&restoredWatch){setConnectNote('Restored the last local mission snapshot while reconnecting to Research Desk.');return;}if(restoredToken){setConnectNote('Restored the approved local connection. Rechecking Research Desk now.');return;}if(restoredWatch){setConnectNote('Restored the last local mission snapshot while reconnecting to Research Desk.');}}
    ...
    const existingPrompt=currentPromptFromContext();if(existingPrompt&&!document.getElementById('connect-note')?.textContent){setConnectNote('Ready to hand off the current first-look prompt into your local desk once the connection is approved.');}
    loadDeskState();
  </script>
</body>
</html>"""
