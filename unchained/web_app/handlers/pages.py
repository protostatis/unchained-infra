...
<body>
  ...
  <script>
    ...
    function scheduleHandshakePoll(statusUrl, requestId, attempt){if(handshakePollTimer) window.clearTimeout(handshakePollTimer);const tries=Number(attempt||0);if(tries>=MAX_HANDSHAKE_POLL_ATTEMPTS){setConnectNote('Timed out waiting for local approval. Try connecting again.');setMissionCreateReady(false);return;}handshakePollTimer=window.setTimeout(async()=>{try{const pollUrl=new URL(statusUrl);pollUrl.searchParams.set('request_id', requestId);const resp=await fetch(pollUrl.toString(),{mode:'cors'});if(!resp.ok) throw new Error('handshake status unavailable');const data=await resp.json();if(data.status==='approved'){approvedHandshakeToken=sanitizeBearerToken(data.session_token);setMissionCreateReady(Boolean(approvedHandshakeToken)&&Boolean(safeOptionalLocalUrl(latestDeskStatus?.handshake?.actions?.mission_create_url)));setConnectNote('Hosted connection approved. Local session token is ready for the next action.');return;}if(data.status==='denied'){approvedHandshakeToken='';setMissionCreateReady(false);setConnectNote('Hosted connection was denied in the local desk.');return;}scheduleHandshakePoll(statusUrl, requestId, tries+1);}catch(_err){setConnectNote('Waiting for local approval...');scheduleHandshakePoll(statusUrl, requestId, tries+1);}},HANDSHAKE_POLL_INTERVAL_MS);}
