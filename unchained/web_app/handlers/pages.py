...
<body>
  ...
  <script>
    ...
    document.getElementById('install-local-research-desk')?.addEventListener('click', async()=>{const button=document.getElementById('install-local-research-desk');if(!button||button.disabled) return;if(!RESEARCH_DESK_INSTALL_AUTHENTICATED){setConnectNote('Sign in to unchainedsky.com first, then retry the Research Desk install.');window.location.href=RESEARCH_DESK_SIGN_IN_URL;return;}installInFlight=true;installRetryAttempts=0;clearInstallRetry();updateInstallButton();setConnectNote('Starting Research Desk install on this machine through the local client...');try{const result=await requestResearchDeskInstall();if(result.kind==='installing'){const launcherPrefix=String(result.data.launcher_prefix||RESEARCH_DESK_LAUNCHER_FALLBACK).trim()||RESEARCH_DESK_LAUNCHER_FALLBACK;finishInstallFlow('Research Desk install started. The local client is running `'+launcherPrefix+' setup`, starting the browser bridge, and starting the local desk automatically. Keep this page open while detection retries.');return;}if(result.kind==='update_required'){if(!Boolean(result.data.update_supported)){throw new Error(String(result.data.error||'Update the local client first, then retry Research Desk install.'));}const clientVersion=String(result.data.client_version||'unknown');const requiredVersion=String(result.data.required_client_version||'0.3.63');setConnectNote('Your local client is still on '+clientVersion+'. Updating it to at least '+requiredVersion+' before retrying Research Desk install...');const updateResult=await requestLocalClientUpdate();if(updateResult.kind==='unauth'){finishInstallFlow('Sign in to unchainedsky.com first, then retry the local client update.');return;}setConnectNote('Local client update started. Waiting for it to reconnect before retrying Research Desk install...');scheduleResearchDeskInstallRetry();return;}if(result.kind==='retryable'){setConnectNote('The local client is reconnecting. Retrying Research Desk install automatically...');scheduleResearchDeskInstallRetry();return;}if(result.kind==='unauth'){finishInstallFlow('Sign in to unchainedsky.com first, then retry the Research Desk install.');return;}}catch(err){finishInstallFlow(String(err?.message||'Could not start the Research Desk install. Make sure the local client is signed in and online, then try again.'));}});
