...
<body>
      ...
      <div class="panel">
        ...
        <div id="mission-watch" class="watch-card">
          ...
          <div class="actions" style="margin-top:12px">
            <a id="mission-watch-link" class="btn secondary disabled" href="http://127.0.0.1:8766/" target="_blank" rel="noreferrer" aria-disabled="true" tabindex="-1">Open Mission</a>
  ...
  <script>
    ...
    function renderMissionWatch(data){const card=document.getElementById('mission-watch');const title=document.getElementById('mission-watch-title');const copy=document.getElementById('mission-watch-copy');const meta=document.getElementById('mission-watch-meta');const link=document.getElementById('mission-watch-link');const nextButton=document.getElementById('run-local-next-step');if(!card||!title||!copy||!meta||!link||!nextButton) return;if(!data||!data.ok){card.classList.remove('visible');missionWatchUrl='';missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';nextButton.classList.add('disabled');nextButton.setAttribute('aria-disabled','true');nextButton.disabled=true;if(missionWatchTimer){window.clearTimeout(missionWatchTimer);missionWatchTimer=null;}return;}card.classList.add('visible');title.textContent=String(data.capsule_name||'mission');copy.textContent=String(data.blocked_reason||data.autopilot_next_label||data.next_step||'Mission is progressing in the local desk.');meta.innerHTML='';[String(data.stage||'planning'), String(data.readiness_status||'planned'), String((Number(data.primary_row_count||0))+' rows'), String(data.accepted_like_fraction ? ('qa '+Math.round(Number(data.accepted_like_fraction||0)*100)+'%') : 'qa pending')].forEach((value)=>{const badge=document.createElement('span');badge.className='pill';badge.textContent=value;meta.appendChild(badge);});const missionUrl=safeOptionalLocalUrl(data.mission_url_abs||data.mission_url||'');if(missionUrl){link.href=missionUrl;link.textContent=Boolean(data.lab_ready)?'Open Lab Notes':'Open Mission';link.classList.remove('disabled');link.setAttribute('aria-disabled','false');link.removeAttribute('tabindex');}else{link.href='#';link.classList.add('disabled');link.setAttribute('aria-disabled','true');link.setAttribute('tabindex','-1');}const nextReady=Boolean(approvedHandshakeToken)&&Boolean(data.can_advance)&&Boolean(safeOptionalLocalUrl(latestDeskStatus?.handshake?.actions?.mission_advance_url));nextButton.classList.toggle('disabled',!nextReady);nextButton.setAttribute('aria-disabled',nextReady?'false':'true');nextButton.disabled=!nextReady;nextButton.textContent=String(data.autopilot_next_label||'Run Next Step');}
    function scheduleMissionWatch(url){if(missionWatchTimer) window.clearTimeout(missionWatchTimer);const safeUrl=safeOptionalLocalUrl(url);if(!safeUrl){missionWatchUrl='';missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';return;}if(missionWatchUrl!==safeUrl){missionWatchAttempts=0;missionWatchStableCount=0;missionWatchLastSignature='';}missionWatchUrl=safeUrl;if(missionWatchAttempts>=MAX_MISSION_WATCH_ATTEMPTS){setConnectNote('Mission watch timed out. Open the local desk to continue from there.');return;}missionWatchTimer=window.setTimeout(async()=>{missionWatchTimer=null;missionWatchAttempts+=1;try{const resp=await fetch(safeUrl,{mode:'cors',credentials:'omit',cache:'no-store',referrerPolicy:'no-referrer',signal:AbortSignal.timeout(5000)});if(!resp.ok){if(resp.status>=500&&missionWatchUrl===safeUrl&&missionWatchAttempts<MAX_MISSION_WATCH_ATTEMPTS){scheduleMissionWatch(safeUrl);return;}setConnectNote('Mission watch stopped after repeated local status failures. Open the local desk to continue.');return;}const data=await resp.json();renderMissionWatch(data);if(!data.ok) return;const readiness=String(data.readiness_status||'');const signature=[String(data.stage||''),readiness,String(data.primary_row_count||0),String(data.next_step||''),String(data.autopilot_next_label||'')].join('|');if(signature===missionWatchLastSignature){missionWatchStableCount+=1;}else{missionWatchLastSignature=signature;missionWatchStableCount=0;}if(['final_ready','blocked'].includes(readiness)) return;if(missionWatchStableCount>=MAX_IDENTICAL_MISSION_STATES){setConnectNote('Mission watch paused because the local state stopped changing. Open the local desk to continue.');return;}scheduleMissionWatch(safeUrl);}catch(_err){if(missionWatchUrl===safeUrl&&missionWatchAttempts<MAX_MISSION_WATCH_ATTEMPTS){scheduleMissionWatch(safeUrl);return;}setConnectNote('Mission watch stopped after repeated local status failures. Open the local desk to continue.');}},MISSION_WATCH_POLL_INTERVAL_MS);}
    ...
    const existingPrompt=currentPromptFromContext();if(existingPrompt){setConnectNote('Ready to hand off the current first-look prompt into your local desk once the connection is approved.');}
    loadDeskState();
  </script>
</body>
</html>"""
