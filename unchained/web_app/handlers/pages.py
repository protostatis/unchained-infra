...
<body>
  ...
  <script>
    ...
    document.getElementById('install-local-research-desk')?.addEventListener('click', async()=>{const button=document.getElementById('install-local-research-desk');if(!button||button.disabled) return;button.disabled=true;button.classList.add('disabled');setConnectNote('Starting Research Desk install on this machine through the local client...');try{const resp=await fetch('/web/chat/install-research-desk',{method:'POST',credentials:'include',cache:'no-store'});const data=await resp.json();if(!resp.ok||!data.ok) throw new Error(String(data.error||'Research Desk install failed to start.'));const launcherPrefix=String(data.launcher_prefix||RESEARCH_DESK_LAUNCHER_FALLBACK).trim()||RESEARCH_DESK_LAUNCHER_FALLBACK;setConnectNote('Research Desk install started. When it finishes, run `'+launcherPrefix+' setup`, then `'+launcherPrefix+' bridge-start`, then `'+launcherPrefix+' serve --open --reload`.');}catch(err){setConnectNote(String(err?.message||'Could not start the Research Desk install. Make sure the local client is signed in and online, then try again.'));}finally{button.disabled=false;button.classList.remove('disabled');}});
