...
<body>
  ...
  <script>
    ...
    function scheduleDeskProbe(){if(deskPollTimer||deskPollAttempts>=MAX_AUTO_POLL_ATTEMPTS) return;deskPollTimer=window.setTimeout(()=>{deskPollTimer=null;deskPollAttempts+=1;loadDeskState({silent:true});},POLL_INTERVAL_MS);}
    ...
    async function loadDeskState(options){const opts=options||{};try{const [statusResp,capsulesResp]=await Promise.all([fetch(STATUS_URL,{mode:'cors'}),fetch(CAPSULES_URL,{mode:'cors'})]);if(!statusResp.ok) throw new Error('local status unavailable');renderStatus(await statusResp.json());if(capsulesResp.ok) renderCapsules(await capsulesResp.json());}catch(err){if(!opts.silent) deskPollAttempts=0;renderMissingDeskState();}}
    document.getElementById('retry-local-desk')?.addEventListener('click', ()=>{deskPollAttempts=0;clearDeskProbe();setStatusNote('Checking again for the local desk...');loadDeskState();});
    loadDeskState();
  </script>
</body>
</html>"""
