...
class TestWebTemplateContracts(unittest.TestCase):
    ...
    def test_research_desk_page_renders_phase3_connect_markers(self):
        ...
        self.assertIn("const MAX_TRANSIENT_HANDSHAKE_NOT_READY = 2;", html)
        ...
        self.assertIn("clearMissionWatchState(false);", html)
        ...
        self.assertIn("const hasRecoveryState=Boolean(approvedHandshakeToken||handshakeInFlight||missionWatchUrl);", html)
        self.assertIn("if(handshakeReady){handshakeNotReadyCount=0;}else{handshakeNotReadyCount+=1;", html)
